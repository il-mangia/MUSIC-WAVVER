...
try:
    from playlists import open_playlist_downloader as PlaylistDownloader
    PLAYLIST_MODULE_AVAILABLE = True
except ImportError as e:
    print(f"⚠️ Modulo playlists non disponibile: {e}")
...
def load_languages():
    if not os.path.exists(LANGUAGES_FILE):
        error_msg = (
            f"❌ ERRORE CRITICO: File delle lingue '{LANGUAGES_FILE}' non trovato!"
        )
        log(error_msg)
...
if platform.system() == "Windows":
    ...
    DEFAULT_DOWNLOAD_DIR = os.path.join(os.path.expanduser("~"), "Music")
useroslanguage = os.getenv("LANG") or "it"
...
DEFAULT_SETTINGS = {
    ...
    "language": useroslanguage[:2] if useroslanguage else "it",
    "max_retries": 3,
    "retry_delay": 1,
    "write_id3": True,
    "audio_quality": "320",
...
def show_ffmpeg_missing_error_linux():
    ...
    ctk.CTkLabel(frame, text="FFmpeg Non Trovato", font=("Segoe UI", 20, "bold")).pack(
        pady=(0, 20)
    )
...
def detect_ffmpeg():
    ...
    if found:
        log(f"✅ ffmpeg trovato nel path: {found}")
        return found
    ...
    if current_os == "linux":
        ...
        messagebox.showerror(
            ...
            "Per installare FFmpeg, riavvia\n"
            "il programma come amministratore.\n\n"
        )
...
class DeezerID3Tagger:
    ...
    def search_track(self, query, limit=5):
        ...
        try:
                ...
                tracks.append(
                    {
                        ...
                        "year": track.get("release_date", "").split("-")[0]
            ...
            log(
                f"✅ Trovati {len(tracks)} risultati su Deezer per query: '{clean_query}'"
            )
            return tracks
        except Exception as e:
    ...
    def calculate_matching_score(
        ...
        if dz_artist_lower in yt_uploader_lower or yt_uploader_lower in dz_artist_lower:
            ...
            if common_words:
                similarity = len(common_words) / max(
                    len(artist_words), len(uploader_words)
                )
        ...
        yt_title_clean = re.sub(
            r"\b(official|video|audio|lyric|lyrics|hd|hq|4k|1080p|720p|mv|clip|music|song|remix|cover|original|extended|radio edit)\b",
            "",
            yt_title_clean,
            flags=re.IGNORECASE,
        )
        ...
        # Confronto esatto
        if (
            yt_title_clean == dz_title_lower
            or dz_title_lower in yt_title_clean
            or yt_title_clean in dz_title_lower
        ):
            score += 20
        else:
        ...
        if youtube_duration and deezer_track.get("duration"):
            ...
            if diff <= 5:  # Differenza di 5 secondi o meno
                score += 40
                log(f"⏱️ Durata perfetta (±5s): +40 punti")
    ...
    def find_best_match(
        ...
        for i, track in enumerate(tracks):
            ...
            scored_tracks.append({**track, "score": score})
        ...
        if best_match and best_match["score"] >= 40:
            log(
                f"✅ Miglior match trovato: '{best_match['title']}' - Score: {best_match['score']}/100"
            )
            return best_match

        log(
            f"⚠️ Nessun buon match trovato (miglior score: {best_match['score'] if best_match else 0}/100)"
        )
        return None
...
def apply_deezer_id3_automatically(parent, filename, filepath, deezer_tagger):
    ...
    if success:
        ...
        messagebox.showwarning(
            "Attenzione", f"Impossibile applicare tag ID3 a:\n{filename}", parent=parent
        )
...
def search_youtube(query, max_results=10, timeout_seconds=30, deezer_tagger=None):
    ...
    t = threading.Thread(
        ...
        args=(query_to_search, max_results, rq, deezer_tagger),
        daemon=True,
    )
    t.start()
...
class YTDownloaderApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Il Mangia's MUSIC WAVVER - V.4.5")
        ...
        log(f"🚀 GUI avviata. Versione: MUSIC WAVVER 4.5")
        ...
        try:
            ...
            if platform.system() == "Windows":
                try:
                    ...
                    myappid = "ilmangia.musicwavver.4.5"
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            title_frame, text=T("welcome"), font=("Segoe UI", 20, "bold")
        ).pack(side="left")
        ...
        ctk.CTkButton(
            button_frame, text=T("open_log"), command=self.open_log, width=80
        ).pack(side="right", padx=(5, 0))
        ctk.CTkButton(
            button_frame, text=T("settings"), command=self.open_settings, width=80
        ).pack(side="right", padx=(5, 0))
        ...
        self.entry = ctk.CTkEntry(
            ...
            placeholder_text="Inserisci URL YouTube o termine di ricerca...",
        )
        self.entry.grid(row=0, column=0, sticky="ew", padx=(10, 5), pady=10)
        self.entry.bind("<Return>", lambda e: self.on_search())

        self.btn_search = ctk.CTkButton(
            search_frame, text=T("search_btn"), command=self.on_search, width=120
        )
        ...
        self.tree.tag_configure(
            "best_match", background="#d4edda", foreground="#155724"
        )  # Verde chiaro
        self.tree.tag_configure(
            "good_match", background="#fff3cd", foreground="#856404"
        )  # Giallo chiaro
        self.tree.tag_configure(
            "no_match", background="#f8f9fa", foreground="#6c757d"
        )  # Grigio chiaro
        self.tree.tag_configure(
            "downloading_tag", background="#3B8ED0", foreground="white"
        )
        ...
        ctk.CTkLabel(controls_frame, text=T("format_label")).pack(
            side="left", padx=(10, 5)
        )
        ctk.CTkOptionMenu(
            controls_frame, variable=self.format, values=["wav", "mp3", "flac"]
        ...
        self.btn_play = ctk.CTkButton(
            controls_frame, text=T("play_btn"), command=self.play_file, state="disabled"
        )
        ...
        ctk.CTkLabel(progress_frame, textvariable=self.status).grid(
            row=0, column=1, padx=(10, 10), pady=10
        )
    ...
    def handle_playlist_prompt(self, url):
        """Gestisce la richiesta di download playlist"""
        if not PLAYLIST_MODULE_AVAILABLE:
            messagebox.showwarning(
                "Funzionalità non disponibile",
                "Il modulo playlist non è disponibile. Apri un issue su GitHub per assistenza.",
                parent=self,
            )
            return
        ...
        if not video_id:
            try:
                ...
                messagebox.showerror(
                    "Errore", f"Impossibile aprire il downloader playlist: {e}"
                )
            return

        # Chiedi se scaricare singolo video o playlist completa
        ...
        ctk.CTkLabel(win, text=T("playlist_prompt_text"), font=("Segoe UI", 12)).pack(
            pady=10
        )
        ...
        ctk.CTkButton(
            button_frame,
            text=T("playlist_prompt_single"),
            command=download_single,
            width=150,
        ).pack(side="left", padx=10)
        ctk.CTkButton(
            button_frame,
            text=T("playlist_prompt_full"),
            command=download_full,
            width=150,
            fg_color="#28a745",
            hover_color="#218838",
        ).pack(side="left", padx=10)
    ...
    def _search_thread(self, q, maxr):
        try:
            ...
            for i, r in enumerate(results):
                ...
                if r.get("deezer_match"):
                    ...
                    if r.get("duration") and r.get("deezer_match", {}).get("duration"):
                        ...
                        if (
                            abs(youtube_dur - deezer_dur) <= 10
                        ):  # Differenza massima 10 secondi
            ...
            self.status.set(
                f"{len(results)} risultati trovati. Miglior match: {best_match_score}/100"
                if best_match_score > 0
                else f"{len(results)} risultati trovati."
            )
    ...
    def _download_thread(self, url, fmt, tree_item_id, result_index):
        try:
            ...
            if (
                fmt == "mp3"
                and SETTINGS.get("write_id3", False)
                and LAST_FILE
                ...
                if result.get("deezer_match"):
                    ...
                    # Scarica copertina
                    cover_data = self.deezer_tagger.download_cover(
                        deezer_match.get("cover_url", "")
                    )
                    ...
                    success = self.deezer_tagger.apply_id3_tags(
                        LAST_FILE, metadata, cover_data
                    )

                    if success:
                        self.queue.put(
                            ("id3_done", (tree_item_id, deezer_match.get("score", 0)))
                        )
                    else:
                        self.queue.put(("id3_failed", tree_item_id))
                else:
                    # Prova ricerca automatica come fallback
                    success = apply_deezer_id3_automatically(
                        self, os.path.basename(LAST_FILE), LAST_FILE, self.deezer_tagger
                    )
                    if success:
    ...
    def open_settings(self):
        ...
        scrollbar = ctk.CTkScrollbar(
            main_frame, orientation="vertical", command=canvas.yview
        )
        ...
        ctk.CTkLabel(
            scrollable_frame,
            text=T("download_folder_label"),
            font=("Segoe UI", 14, "bold"),
        ).grid(row=row_counter, column=0, sticky="w", pady=(20, 5), padx=20)
        row_counter += 1
        ...
        self.dir_label = ctk.CTkLabel(
            dir_frame, text=SETTINGS["download_dir"], wraplength=480
        )
        self.dir_label.grid(row=0, column=0, sticky="w", padx=10, pady=8)
        ctk.CTkButton(
            dir_frame,
            text=T("change_folder"),
            command=lambda: self.change_dir(win),
            width=80,
        ).grid(row=0, column=1, padx=10, pady=8)

        # Qualità audio MP3
        ctk.CTkLabel(
            scrollable_frame, text="Qualità Audio MP3", font=("Segoe UI", 14, "bold")
        ).grid(row=row_counter, column=0, sticky="w", pady=(20, 5), padx=20)
        row_counter += 1
        ...
        # Lingua
        ctk.CTkLabel(
            scrollable_frame, text=T("language_label"), font=("Segoe UI", 14, "bold")
        ).grid(row=row_counter, column=0, sticky="w", pady=(20, 5), padx=20)
        row_counter += 1
        ...
        # Tema
        ctk.CTkLabel(
            scrollable_frame, text=T("theme_label"), font=("Segoe UI", 14, "bold")
        ).grid(row=row_counter, column=0, sticky="w", pady=(20, 5), padx=20)
        row_counter += 1
        ...
        # ID3 Tag
        ctk.CTkLabel(
            scrollable_frame, text="ID3 Tag", font=("Segoe UI", 14, "bold")
        ).grid(row=row_counter, column=0, sticky="w", pady=(20, 5), padx=20)
        row_counter += 1

        self.id3_var = ctk.BooleanVar(value=SETTINGS.get("write_id3", False))
        id3_check = ctk.CTkCheckBox(
            scrollable_frame,
            text=T("id3_enable_label"),
            variable=self.id3_var,
            checkbox_width=20,
            checkbox_height=20,
        )
        ...
        ctk.CTkLabel(
            scrollable_frame, text=T("speed_limit_label"), font=("Segoe UI", 14, "bold")
        ).grid(row=row_counter, column=0, sticky="w", pady=(20, 5), padx=20)
        row_counter += 1
        ...
        ctk.CTkButton(
            ...
            hover_color="#218838",
            height=40,
        ).grid(row=row_counter, column=0, sticky="ew", padx=20, pady=20)
        row_counter += 1
    ...
    def change_dir(self, parent_win):
        d = filedialog.askdirectory(
            initialdir=SETTINGS["download_dir"], title=T("select_download_folder")
        )
        if d:
    ...
    def save_settings(self, win):
        ...
        messagebox.showinfo(
            "Impostazioni", "Impostazioni salvate con successo!", parent=win
        )
        win.destroy()
